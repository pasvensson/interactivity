...
function processPoses(poses) {
  ...
  if (poses.length == 1 && poses[0].score > 0.3) {
    ...
    const nose = getKeypointPos(poses, 'nose');
    ...
    if (rightWrist != null && pointsAreInFrame(rightWrist,rightWrist)) {
      ...
      c.fillText('Distance between arms: ' + distance, 100, 10);
       ...
       console.log(pannerL.pan.value);
       console.log(pannerR.pan.value);
    }
...
function startSound() {
  if (!soundIsStarted) {
    ...
    pannerR = new Tone.Panner(0).connect(filter);
    pannerL = new Tone.Panner(0).connect(filter);
    noiseR = new Tone.Noise("pink").connect(pannerR).start();
    noiseL = new Tone.Noise("pink").connect(pannerL).start();
    //triangle2 = new Tone.FatOscillator(440, "triangle", 80).connect(pannerR).start();
    //triangle1 = new Tone.FatOscillator(400, "triangle", 80).connect(pannerL).start();
    //noiseR.volume.value = -8;
    //noiseL.volume.value = -8;
    soundIsStarted = true;
  }
}