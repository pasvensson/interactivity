...
function renderFrame() {
  ...
  for (let pixelIndex = 0; pixelIndex < totalPixels; pixelIndex++) {
    ...
    frame.data[pixelIndex * 4 + 0] = 64 * Math.trunc(r / 64);
    frame.data[pixelIndex * 4 + 1] = 64 * Math.trunc(g / 64);
    frame.data[pixelIndex * 4 + 2] = 64 * Math.trunc(b / 64);
  }
  ...
  pixelate(offscreenC,canvasEl.width, canvasEl.height,0,0)
  
  // Draw buffer to the visible canvas
  //c.drawImage(offscreenCanvasEl, 0, 0,w,h, 0, 0, canvasEl.width, canvasEl.height);
...
// The pixelation function is from
