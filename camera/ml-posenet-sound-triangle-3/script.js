...
function processPoses(poses) {
  ...
  if (poses.length == 1 && poses[0].score > 0.3) {
    ...
    if (leftWrist != null && rightWrist != null && pointsAreInFrame(leftWrist,rightWrist)) {
...
//       slider2 = document.getElementById("triangle2");
       triangle1.frequency.value = (leftWrist.x + rightWrist.x) / 7;
       triangle2.frequency.value = (rightWrist.y + leftWrist.y) / 7;
//       filter.frequency.value = distance + (leftWrist.x + rightWrist.x + rightWrist.y + leftWrist.y ) / 7;
       filter.frequency.value = leftEye.x - 320 + (leftWrist.x + rightWrist.x + rightWrist.y + leftWrist.y ) / 7;
       // noise.volume.value = -1 * (leftEye.x - rightEye.x) / 2;
    }

    if ( pointsAreInFrame(leftWrist,rightWrist) ) {
      //   console.log("IN");
        triangle1.volume.value = 10;
        triangle2.volume.value = 10;
    } else {
      //   console.log("OUT");
        triangle1.volume.value = -100;
        triangle2.volume.value = -100;
      }
...
function startSound() {
  if (!soundIsStarted) {
    ...
    */
    ...
    triangle2 = new Tone.FatOscillator(440, "triangle", 80).connect(filter).start();
    triangle1 = new Tone.FatOscillator(400, "triangle", 80).connect(filter).start();
