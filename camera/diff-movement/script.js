...
function renderFrame() {
  ...
  diffCountPercent = 100 - Math.floor(100 * (diffCount / totalPixels));
  c.fillStyle = 'white';
  c.font = '48px "Fira Code", Monaco, "Andale Mono", "Lucida Console", "Bitstream Vera Sans Mono", "Courier New", Courier, monospace';
  diffTime = Date.now()-lastUpdateTime;
  lastUpdateTime = Date.now();
  var presentsDetection = 
  c.fillText(307200-diffCount + '  ' + diffCountPercent + '%', 100, 100);
  var presentsDetection = 307200-diffCount;
  
    
//  c.fillText(Math.floor(1000/diffTime) + ' fps' , 100, 100);
  ...
  if (now-oldFrameCapturedAt < 2000) {
    keepFrame = false;
    if (presentsDetection > 1) {
      console.log('someone is present')
    } else{
